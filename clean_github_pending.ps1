...
function Quote-GitHubPath([string]) {
     = .Trim().TrimStart('/')
     = .Split('/')
     = foreach ( in ) { [System.Uri]::EscapeDataString() }
    return ( -join '/')
}
...
try {
    ...
    if (.Exception.Response.StatusCode.value__ -eq 404) {
        ...
        Write-Host ('Failed to connect to GitHub: ' + .Exception.Message) -ForegroundColor Red
        exit 1
    }
}
...
Write-Host ('Found ' + .Count + ' file(s) in GitHub pending folder:') -ForegroundColor Yellow
foreach ( in ) {
    Write-Host ('  - ' + .name + ' (' + .size + ' bytes)') -ForegroundColor Gray
}

 = Read-Host ('
Do you want to permanently delete all ' + .Count + ' files from GitHub? (y/N)')
if ( -ne 'y' -and  -ne 'Y') {
...
foreach ( in ) {
     = Quote-GitHubPath .path
     = 'https://api.github.com/repos/' +  + '/contents/' + 
     = @{
        message = ('Delete ' + .name + ' via cleanup script')
        sha     = .sha
        branch  = 
    } | ConvertTo-Json
    ...
    try {
        ...
        Write-Host ('  [FAILED]  ' + .name + ': ' + .Exception.Message) -ForegroundColor Red
    }
}

Write-Host ''
Write-Host ('Cleanup completed: ' +  + ' of ' + .Count + ' file(s) deleted.') -ForegroundColor Cyan
